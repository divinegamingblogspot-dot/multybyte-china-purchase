...
/* Lock the endpoint so later feature code cannot silently replace it. */
try{
...
/* Last-line transport guard: even legacy app.js code using its own lexical API
 * cannot send an Apps Script request to the old/wrong endpoint. */
  ...
  try{
    if(node && node.tagName==='SCRIPT' && typeof node.src==='string' && node.src.indexOf('script.google.com/macros/s/')!==-1){
      ...
      if(u.pathname.endsWith('/exec')){
        const incoming=u.toString();
        if(incoming.indexOf(CANONICAL)===-1){
...
/* Protected JSONP gateway with retries. No feature should bypass this. */
function gateway(params,timeout){
 ...
 return new Promise((resolve,reject)=>{
  ...
  const run=()=>{
   ...
   script=document.createElement('script');script.async=true;script.src=CANONICAL+'?'+q.toString();
...
function bindLogin(){
 ...
 if(ar){const n=ar.cloneNode(true);ar.replaceWith(n);n.addEventListener('click',()=>{window.__mbLoginRole='admin';n.classList.add('active');n.classList.remove('active');$('vendorRole')?.classList.remove('active')})}
...
async function loadVendorFixed(){try{const [r,p]=await Promise.all([gateway({action:'vendorData',token:token()}),gateway({action:'vendorPurchaseOrders',token:token()})]);window.products=r?.products||[];window.pos=p?.purchaseOrders||p?.data||[];if(typeof drawVendor==='function')drawVendor();if(typeof drawPO==='function')drawPO('vendorPO',window.pos);if(typeof drawDashboard==='function')drawDashboard()}catch(e){toastSafe('Vendor gateway failed: '+e.message)}}
...
async function loadVendorsFixed(){try{const r=await gateway({action:'adminVendors',token:token()});if(r?.success&&Array.isArray(r.vendors))window.vendors=r.vendors;if(typeof loadVendors==='function'){}return r}catch(e){toastSafe('Vendors failed: '+e.message)}}
...
function install(){
 window.loadAdminFixed=loadAdminFixed;window.loadVendorFixed=loadVendorFixed;window.loadPOFixed=loadPOFixed;window.loadAdmin=loadAdminFixed;window.loadVendor=loadVendorFixed;window.loadPO=loadPOFixed;window.loadVendors=loadVendorsFixed;window.addPOSku=addSkuFixed;window.createPO=createPOFixed;
 bindLogin();
