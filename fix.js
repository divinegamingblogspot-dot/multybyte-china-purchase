...
window.__MB_GATEWAY_VERSION='protected-v5';window.__MB_GATEWAY_URL=CANONICAL;

/* Protect old app.js JSONP calls and force every Apps Script request to the
   one canonical deployment. The old 800ms onload failure is neutralized. */
  ...
  try{
    if(node&&node.tagName==='SCRIPT'&&typeof node.src==='string'&&node.src.indexOf('script.google.com/macros/s/')!==-1){
      ...
      if(u.pathname.endsWith('/exec')){
        ...
        if(changed){node.onload=null;node.onerror=null;}
      }
    }
  }catch(e){}
...
/* app.js keeps its working arrays in top-level lexical variables. Those are
   intentionally bridged here through indirect global eval so the protected
   transport cannot leave the UI showing an empty state after a successful API
   response. No feature logic is replaced; the existing render/edit functions
   continue to operate on their original app.js state. */
function syncAppState(p,v,m,o){
  window.__MB_STATE_PRODUCTS=Array.isArray(p)?p:[];
  window.__MB_STATE_VENDORS=Array.isArray(v)?v:[];
  window.__MB_STATE_MASTER=Array.isArray(m)?m:[];
  window.__MB_STATE_POS=Array.isArray(o)?o:[];
  try{
    window.eval('products=window.__MB_STATE_PRODUCTS; vendors=window.__MB_STATE_VENDORS; masterProducts=window.__MB_STATE_MASTER; pos=window.__MB_STATE_POS;');
  }catch(e){console.error('Multybyte state bridge failed',e)}
}
...
async function loadAdminFixed(){try{const[a,v,m]=await Promise.all([gateway({action:'adminProducts',token:token()}),gateway({action:'adminVendors',token:token()}),gateway({action:'data',token:token()})]);const productsData=a?.products||a?.data||[];const vendorsData=v?.vendors||[];const masterData=m?.products||m?.data||[];syncAppState(productsData,vendorsData,masterData,[]);if(typeof drawAdminStats==='function')drawAdminStats();if(typeof drawAdminProducts==='function')drawAdminProducts();if(typeof drawDashboard==='function')drawDashboard();await loadPOFixed()}catch(e){toastSafe('Admin data failed: '+e.message)}}
async function loadVendorFixed(){try{const[r,p]=await Promise.all([gateway({action:'vendorData',token:token()}),gateway({action:'vendorPurchaseOrders',token:token()})]);const productsData=r?.products||r?.data||[];const posData=p?.purchaseOrders||p?.data||[];syncAppState(productsData,[],[],posData);if(typeof drawVendor==='function')drawVendor();if(typeof drawPO==='function')drawPO('vendorPO',posData);if(typeof drawDashboard==='function')drawDashboard()}catch(e){toastSafe('Vendor data failed: '+e.message)}}
...
async function loadVendorsFixed(){try{const r=await gateway({action:'adminVendors',token:token()});if(r?.success&&Array.isArray(r.vendors)){window.__MB_STATE_VENDORS=r.vendors;try{window.eval('vendors=window.__MB_STATE_VENDORS;')}catch(e){}}return r}catch(e){toastSafe('Vendors failed: '+e.message)}}
...
function install(){window.loadAdminFixed=loadAdminFixed;window.loadVendorFixed=loadVendorFixed;window.loadPOFixed=loadPOFixed;window.loadAdmin=loadAdminFixed;window.loadVendor=loadVendorFixed;window.loadPO=loadPOFixed;window.loadVendors=loadVendorsFixed;window.addPOSku=addSkuFixed;window.createPO=createPOFixed;bindLogin();const poRefresh=$('adminPORefresh');if(poRefresh){const n=poRefresh.cloneNode(true);poRefresh.replaceWith(n);n.addEventListener('click',loadPOFixed)}const poAdd=$('poAdd');if(poAdd){const n=poAdd.cloneNode(true);poAdd.replaceWith(n);n.addEventListener('click',addSkuFixed)}const poCreate=$('poCreate');if(poCreate){const n=poCreate.cloneNode(true);poCreate.replaceWith(n);n.addEventListener('click',createPOFixed)}}
