 ...
 * Keep transport isolated here. app.js contains legacy lexical api(), so simply replacing
 * window.api is NOT enough. This file explicitly rebinds the login/control entry points.
 */
(function(){'use strict';
...
/* One transport for every API call. No script.onerror-only logic and no cached URLs. */
...
/* Login must be rebound because app.js's login() closes over its old lexical api(). */
async function loginSafe(){
 ...
 if($('loginMsg'))$('loginMsg').textContent='Connecting…';
 try{
  ...
  if(r.role==='admin'&&typeof window.loadAdminFixed==='function')window.loadAdminFixed();
  else if(r.role==='vendor'&&typeof window.loadVendorFixed==='function')window.loadVendorFixed();
 }catch(e){if($('loginMsg'))$('loginMsg').textContent=e.message||'Gateway unavailable'}
 finally{if(b)b.disabled=false}
}
function bindLogin(){
 const old=$('loginBtn');if(!old)return;
 /* clone removes the original addEventListener(login) from app.js */
...
async function loadAdminFixed(){
 try{
  const [a,v,m]=await Promise.all([gateway({action:'adminProducts',token:token()}),gateway({action:'adminVendors',token:token()}),gateway({action:'data',token:token()})]);
  window.products=(a?.products||[]);window.masterProducts=(m?.products||m?.data||[]);window.vendors=(v?.vendors||[]);
  if(typeof drawAdminStats==='function')drawAdminStats();if(typeof drawAdminProducts==='function')drawAdminProducts();if(typeof drawDashboard==='function')drawDashboard();
  await loadPOFixed();
 }catch(e){toastSafe('Admin gateway failed: '+e.message)}
}
async function loadVendorFixed(){
 try{const r=await gateway({action:'vendorData',token:token()});window.products=r?.products||[];if(typeof drawVendor==='function')drawVendor();if(typeof drawDashboard==='function')drawDashboard()}
 catch(e){toastSafe('Vendor gateway failed: '+e.message)}
}
async function loadPOFixed(){
 try{const r=await gateway({action:'purchaseOrders',token:token()});if(!r?.success)throw Error(r?.message||'Purchase orders failed');window.pos=r.purchaseOrders||r.data||[];if(typeof drawPO==='function')drawPO('adminPO',window.pos);if(typeof drawDashboard==='function')drawDashboard();return r}
 catch(e){toastSafe('Purchase orders failed: '+e.message)}
}
async function loadVendorsFixed(){try{const r=await gateway({action:'adminVendors',token:token()});window.vendors=r?.vendors||[];if(typeof loadVendors==='function'&&loadVendors!==loadVendorsFixed)return r;return r}catch(e){toastSafe('Vendors failed: '+e.message)}}
async function addSkuFixed(){const sku=$('poSku')?.value.trim();if(!sku)return;try{const r=await gateway({action:'purchaseLookup',token:token(),sku});if(!r?.success)throw Error(r?.message||'SKU lookup failed');const p=r.product||r.data;if(!p?.sku)throw Error('SKU not found in Website Listing');window.poItems=window.poItems||[];if(poItems.some(x=>String(x.sku).toLowerCase()===String(p.sku).toLowerCase()))throw Error('SKU already added');poItems.push({...p,quantity:1});$('poSku').value='';if(typeof renderPOItems==='function')renderPOItems();if($('poLookup'))$('poLookup').textContent='Added '+(p.productName||p.name||p.sku)}catch(e){toastSafe(e.message)}}
async function createPOFixed(){const vs=typeof selectedPOVendors==='function'?selectedPOVendors():[];if(!vs.length)return toastSafe('Select at least one vendor.');if(!(window.poItems||[]).length)return toastSafe('Add at least one SKU.');const b=$('poCreate');if(b)b.disabled=true;try{for(const v of vs){const r=await gateway({action:'savePurchaseOrder',token:token(),vendorId:v.id,vendorName:v.name,items:JSON.stringify(poItems.map(p=>({sku:p.sku,quantity:p.quantity,remarks:p.remarks||''})))});if(!r?.success)throw Error(r?.message||'Purchase order failed')}toastSafe('Purchase order created successfully.');if(typeof closePO==='function')closePO();await loadPOFixed()}catch(e){toastSafe(e.message)}finally{if(b)b.disabled=false}}
function install(){
 window.loadAdminFixed=loadAdminFixed;window.loadVendorFixed=loadVendorFixed;window.loadPOFixed=loadPOFixed;
 window.loadAdmin=loadAdminFixed;window.loadPO=loadPOFixed;window.loadVendors=loadVendorsFixed;window.addPOSku=addSkuFixed;window.createPO=createPOFixed;
 bindLogin();
